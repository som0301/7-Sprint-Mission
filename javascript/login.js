...
function updateLoginBtn() {
  loginBtn.disabled = !(emailIsValid && passwordIsValid);
  btnActive ();
}
...
function btnActive () {
  ...
  console.log("btnActive called", emailIsValid, passwordIsValid);
  if(loginBtn.disabled) { // 로그인버튼 비활성화
    loginBtn.style.backgroundColor = ""; // 초기 색상으로 재설정
    // 비활성화 상태에서는 이벤트 리스너 제거
    loginBtn.removeEventListener('click', redirectToItems);
  } else { // 로그인 버튼 비활성화
    console.log('emailIsValid: ' + emailIsValid, 'passwordIsValid: ' + passwordIsValid);
    loginBtn.style.backgroundColor = "#3692FF";
    loginBtn.addEventListener('click', redirectToItems);
  }
}
...
togglePassword.addEventListener('click', function() {
  if(passwordInput.type === 'password') { // input type이 password 일 때 
    passwordInput.type = 'text'; // text로 type 변경
    togglePassword.classList.add('fa-eye'); // 눈 모양 아이콘 보이게
    togglePassword.classList.remove('fa-eye-slash');
  } else { // input type이 text일 때 
    passwordInput.type = 'password'; //password로 변경
    togglePassword.classList.add('fa-eye-slash'); // 눈 slash 보이게
    togglePassword.classList.remove('fa-eye'); //눈 모양 제거
  }
})