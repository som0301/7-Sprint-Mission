import { validateEmail } from './validationUtils.js'
...
function getEmailValidateMsg(inputValue, checkFunc=null) {
  if(inputValue === '') {
    return '이메일을 입력해주세요.';
...
function getNicknameValidateMsg(inputValue, checkFunc=null) {
  if(inputValue === '') {
    return '닉네임을 입력해주세요.';
  } else {
    return  '';
...
function getPwValidateMsg(inputValue, checkFunc=null) {
  if(inputValue === '') {
...
function getPwCheckValidateMsg(inputValue, checkFunc=null) {
  if(inputValue === '') {
    return '비밀번호를 입력해 주세요.';
  } else if (inputValue.length < 8) {
    return '비밀번호를 8자 이상 입력해주세요.';
